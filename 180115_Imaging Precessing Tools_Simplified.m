...
%%
    Lateral_Scanning_Speed=0.48;   %mm/sec
    ...
    Stitched_Y_ROI=[];
%% Line-scan Parameter
FF_Length=-1;                % 0 for all length, -1 for not doing FF
...
%%
root_folder_path='C:\TuanShu\180103_Line Scan opt\';
last_folder_name='CCDZ opt_R2';
Number=[];
%%
Task='CCDZ';    %CCDZ or PZT
...
if strcmp(Task,'CCDZ')
    ...
    Axial_ROI=[100 350];%[100 350];    %micron
elseif strcmp(Task,'PZT')
    If_Sharpness_Est=0;
    If_Lateral_Sharpness_Only=1;
    AVE_postHilbert=4*2*2;
...
if If_LineScan == 1
    ...
    elseif If_Demodulation == 2 % Demodulation along Y with pixel shifting
        ...
        elseif FF_Length == 0
            FF_3D=repmat(mean(Image_Stack_Temp,3),[1 1 size(Image_Stack_Temp,3)]);
        elseif FF_Length == -1
            FF_3D=ones([size(Image_Stack_Temp,1) size(Image_Stack_Temp,2) size(Image_Stack_Temp,3)]);
        end
...
%%
    imagesc(Stitched_Image);
    elseif If_Demodulation == 3 % Demodulation along T with pixel shifting (
