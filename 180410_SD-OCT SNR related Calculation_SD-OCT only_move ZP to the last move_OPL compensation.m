...
subplot(1,1,1)
plot(Wavelength_Array,TSRI(Wavelength_Array,'BK7')); %'BK7' 'S-LAL13' 'S-BSM81' 'S-PHM52'
...
Thickness_List=[5 14.93 0.6+3+4+17.2];%+22.615];%+17.2];% 0 0];%[0.6+3+4+17.2 14.93 5 0 0];%[0.6+3+4+17.2 14.93 5 3.42 0];    %array, mm, single-trip thickness
...
DC_Material={'Silica' 'BK7' 'S-PHM52' 'S-BSM81' 'S-LAL13'}%{'Silica' 'L-LAL13' 'S-TIH53' 'S-NPH2' 'BK7' 'S-LAL13' 'S-BSM81' 'S-PHM52'};
Thickness_Tol=600;   %micron, +/-  250
delta_Thickness=600;%50
...
DC_RI_Mean=zeros(1,length(DC_Material));
for q=1:length(DC_Material)
    ...
    DC_RI_Mean(q)=mean(TSRI(Wavelength_Array,DC_Material{q}));
%     subplot(2,1,1)
...
Air_Spacing=Sample_Arm_OPL-DC_Thickness_Est/1000.*DC_RI_Mean
Total_Length=DC_Thickness_Est/1000+Air_Spacing
%Sample_Arm_OPL
