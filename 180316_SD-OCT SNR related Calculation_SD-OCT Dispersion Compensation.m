...
subplot(1,1,1)
plot(Full_Frequency_Array/1E12,Full_Complex_Spectrum_dn);
xlabel('Optical Frequency (THz)');
ylabel('Digital Number (DN)');
...
Material_List={'Water' 'L-LAL13' 'S-TIH53' 'S-NPH2' 'Silica'}; %Cell
Thickness_List=[0.6+3+4+17.2 14.93 5 0 0];%[0.6+3+4+17.2 14.93 5 0 0];%[0.6+3+4+17.2 14.93 5 3.42 0];    %array, mm, single-trip thickness
Phase_Spectrum_Modi=TSDispersionEst(Material_List,Thickness_List,Wavelength_Array,Central_Wavelength_Index);


%% Generate Dispersed Spectrum
Dispersed_Spectrum_dn=Complex_Spectrum_dn.*exp(i*Phase_Spectrum_Modi);

subplot(1,1,1)
plot(Frequency_Array/1E12,Complex_Spectrum_dn);
xlabel('Optical Frequency (THz)');
ylabel('Digital Number (DN)');
xlim([min(Frequency_Array/1E12) max(Frequency_Array/1E12)]);
...
S_Dispersed_Full=TSZPFFT(Dispersed_Spectrum_dn,Full_Frequency_Array,Min_Frequency_Index);
...
DC_Material='S-TIH53';%'S-TIH53';
DC_Thickness_Est=TSDCtEst(Phase_Spectrum_Modi,Wavelength_Array,DC_Material);


%% Dispersion Compensation with Selected DC
Thickness_Tol=50;   %micron, +/-
delta_Thickness=50;
DC_Thickness_Array=DC_Thickness_Est+[-1*Thickness_Tol:delta_Thickness:Thickness_Tol];
PSF_FWHM_Array=zeros(length(DC_Thickness_Array),1);
for p=1:length(DC_Thickness_Array)
    DC_Phase_Spectrum=TSRI(Wavelength_Array,DC_Material).*DC_Thickness_Array(p)./Wavelength_Array*2*pi*2;  %roundtrip
    ...
    Comepensated_Spectrum_dn=Dispersed_Spectrum_dn.*exp(-i.*DC_Phase_Spectrum_Modi);
    if DC_Thickness_Array(p) == DC_Thickness_Est
        Est_Comepensated_Spectrum_dn=Comepensated_Spectrum_dn;
    end
    S_Compensated_Full=TSZPFFT(Comepensated_Spectrum_dn,Full_Frequency_Array,Min_Frequency_Index);
    S_Compensated=S_Compensated_Full(1:length(Position_micron));
    PSF_FWHM_Array(p)=Position_micron(abs(find(abs(S_Compensated)>0.5*max(abs(S_Compensated)),1,'first')-find(abs(S_Compensated)>0.5*max(abs(S_Compensated)),1,'last')));
...
plot(DC_Thickness_Array/1E3,PSF_FWHM_Array/TSRI(Center_Wavelengh,'Water'),'LineWidth',4);
xlabel(sprintf('Thickness of %s Block (mm)',DC_Material));
ylabel('Axial Resolution in Water (\mum)');
xlim([min(DC_Thickness_Array/1E3) max(DC_Thickness_Array/1E3)]);
if min(PSF_FWHM_Array/TSRI(Center_Wavelengh,'Water'))~=max(PSF_FWHM_Array/TSRI(Center_Wavelengh,'Water'))
    ylim([min(PSF_FWHM_Array/TSRI(Center_Wavelengh,'Water')) max(PSF_FWHM_Array/TSRI(Center_Wavelengh,'Water'))]);
end
%% Change Material Length (i.e. Water of Eye), in SAMPLE ARM
Varying_Material='Water';
Varying_Thickness_Array=17.2*1E3;%([28.03 22.615 17.2 14.28 11.36]-17.2)*1E3; %micron
PSF_FWHM_Array_2=zeros(length(Varying_Thickness_Array),1);
...
for p=1:length(Varying_Thickness_Array)
    ...
    Varied_Comepensated_Spectrum_dn=Est_Comepensated_Spectrum_dn.*exp(i.*Varying_Phase_Spectrum_Modi);

    S_Varied_Full=TSZPFFT(Varied_Comepensated_Spectrum_dn,Full_Frequency_Array,Min_Frequency_Index);
    S_Varied=S_Varied_Full(1:length(Position_micron));
    PSF_FWHM_Array_2(p)=Position_micron(abs(find(abs(S_Varied)>0.5*max(abs(S_Varied)),1,'first')-find(abs(S_Varied)>0.5*max(abs(S_Varied)),1,'last')));
...
plot(Varying_Thickness_Array/1E3+17.2,PSF_FWHM_Array_2/TSRI(Center_Wavelengh,'Water'),'LineWidth',4);
...
%ylim([min(PSF_FWHM_Array_2/TSRI(Center_Wavelengh,'Water')) max(PSF_FWHM_Array_2/TSRI(Center_Wavelengh,'Water'))]);

Diopter_Array=[-20 -10 0 10 20];

plot(Diopter_Array,PSF_FWHM_Array_2/TSRI(Center_Wavelengh,'Water'),'LineWidth',4);
xlabel('屈光度(diopter)');
ylabel('Axial Resolution in Water (\mum)');
...
Sigma=Optical_Frequency_Resolution/2/(2*log(2))^0.5;    %Used for Filtering, 但先不用
Input_Spectrum=real(Est_Comepensated_Spectrum_dn);    %Varied_Comepensated_Spectrum_dn, Comepensated_Spectrum_dn, Dispersed_Spectrum_dn, Complex_Spectrum_dn
...
subplot(1,1,1)
plot(Sensor_Wavelength_Array,Sensor_Spectrum_AVE,Sensor_Wavelength_Array,Sensor_Spectrum_noAVE);
