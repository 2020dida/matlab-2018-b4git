...
%%
    Lateral_Scanning_Speed=0.24;   %mm/sec
...
%%
root_folder_path='C:\TuanShu\180104_Pork Test\0.4A_3Hz\';
last_folder_name='0.24mmsec';
Number=[1];
%%
...
%% Line Scan
if If_LineScan == 1
    If_Demodulation=2;          %0 for no demodulation, 1 for frame shifting after demodulation, 2 for frame shifting before demodulation
    If_Demosaic=1;
    If_Manual_Shift=0;
    Manual_Shift_Value=1;
    ...
    elseif If_Demodulation == 2
        ...
        Lateral_Scanning_Speed_Pixel=Lateral_Scanning_Speed*1000/Lateral_SR;
        if If_Manual_Shift == 0
            Frame_Spacing_per_Lateral_Pixel_PreNPoint=round(Frame_Rate/Lateral_Scanning_Speed_Pixel); 
            
        elseif If_Manual_Shift == 1
            Frame_Spacing_per_Lateral_Pixel_PreNPoint=Manual_Shift_Value;
        end
...
%     end
    ...
    elseif Overlapping_Mode ==2
        if If_Manual_Shift == 0
            imwrite(Stitched_Image,[Data_Save_Folder '\' folder_list(QQQ).name sprintf('_Line_Scan_Averaged_O%g_DMode%g.png',Order,If_Demodulation)],'png');
        elseif If_Manual_Shift == 1
            imwrite(Stitched_Image,[Data_Save_Folder '\' folder_list(QQQ).name sprintf('_Line_Scan_Averaged_O%g_DMode%g_S%g.png',Order,If_Demodulation,Manual_Shift_Value)],'png');
        end
    end
