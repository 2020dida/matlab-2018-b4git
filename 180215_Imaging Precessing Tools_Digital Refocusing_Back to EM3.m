...
If_Save_Raw3D=0;
If_Reverse=1;
File_Reading_Range=[7500 8000]; %9000附近為glass
...
Image_Stack_FFTXY_Corrected_Refocused=Image_Stack_FFTXY_Corrected.*exp(i*C*(1E-9).*(Z_Grid-Z0).*(((KX_Grid-KX0).^2)+((KY_Grid-KY0).^2)));
%Image_Stack_Complex_Corrected_Refocused=ifft(ifft(Image_Stack_FFTXY_Corrected_Refocused,[],2),[],1);

%% Lateral Bandpass
If_Normalized_Spatial_Spectrum=0;
...
Image_Stack_Complex_Corrected_Refocused_Masked=ifft(ifft(Image_Stack_FFTXY_Corrected_Refocused_Masked,[],2),[],1);
%%
    Cmax=40; %30 500
    Cmin=2    
    Frame_Thickness=25;
    Start_Frame=350-round(Frame_Thickness/2);
    Image_Stack_Enface_Corrected=mean(abs(Image_Stack_Complex_Corrected_Refocused_Masked(:,:,Start_Frame:(Start_Frame+Frame_Thickness-1))),3)';
    ...
    axis off
    axis equal
    imwrite(Image_Stack_Enface_Corrected,[Data_Save_Folder sprintf('%s_Corrected_Start%g_Thick%g_C%0g_MDia%g_%g.tif',folder_list(QQQ).name,Start_Frame,Frame_Thickness,C,Spatial_Spectrum_Mask_Diameter_Pixel,If_Normalized_Spatial_Spectrum)],'tiff');
%% B-scan View
Y_index=300;
Start_Frame=200;
End_Frame=500;
imagesc(squeeze(abs(Image_Stack_Complex_Corrected_Refocused_Masked(:,Y_index,Start_Frame:End_Frame)))');
    %% Color for Depth
    Start_Frame=1;
    End_Frame=500;
    Blur_Sigma=2;
    Axial_Smooth_Before_Max=48;

    Image_Intensity=mean(abs(Image_Stack_Complex_Corrected_Refocused_Masked(:,:,Start_Frame:End_Frame)),3);
    Image_Intensity_Max=max(TSBinning(abs(Image_Stack_Complex_Corrected_Refocused_Masked(:,:,Start_Frame:End_Frame)),3,Axial_Smooth_Before_Max),[],3);

    %Image_Intensity_Blur=imgaussfilt(Image_Intensity,Blur_Sigma);
    Find_Z_Grid=repmat(reshape([1:(End_Frame-Start_Frame+1)],[1 1 (End_Frame-Start_Frame+1)]),[[size(Image_Stack_Complex_Corrected_Refocused_Masked,1) size(Image_Stack_Complex_Corrected_Refocused_Masked,2) 1]]);
    Image_Depth=mean(abs(Image_Stack_Complex_Corrected_Refocused_Masked(:,:,Start_Frame:End_Frame)).*Find_Z_Grid,3)./Image_Intensity;
    Image_Axial_Diff=abs(diff(abs(Image_Stack_Complex_Corrected_Refocused_Masked(:,:,Start_Frame:End_Frame)),1,3));
...
%% hsv to rgb
    ...
    Color_max=650.01;    
    Color_min=110;
    Color_Index_Range=[0.89 1.63];
    Sat=1;
    HSV_Image=ones(size(Image_Depth,1),size(Image_Depth,2),3).*Sat;
    Color_Map=(Image_Depth_Blur-Color_min)/(Color_max-Color_min);
    ...
    HSV_Image(:,:,3)=(Image_Intensity_Max-V_min)/(V_max-V_min);
    ...
    axis equal
    %%
    imwrite(Image_Stack_Enface_Corrected,[Data_Save_Folder sprintf('%s_Corrected_Start%g_Thick%g_C%0g_MDia%g.tif',folder_list(QQQ).name,Start_Frame,Frame_Thickness,C,Spatial_Spectrum_Mask_Diameter_Pixel)],'tiff');
    imwrite(RGB_Image,[Data_Save_Folder sprintf('%s_Corrected_Start%g_End%g_C%0g_MDia%g_Color.tif',folder_list(QQQ).name,Start_Frame,End_Frame,C,Spatial_Spectrum_Mask_Diameter_Pixel)],'tiff');
