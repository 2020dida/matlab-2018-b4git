...
If_Save_Raw3D=0;
If_Reverse=1;
File_Reading_Range=[7001 7500];   %[1 11000]
...
last_folder_name='180209_EM3 Data for Processing\';
Number=[5];
%%
Task='ColorCoding';    %CCDZ or PZT 
%% Basic Operations
...
elseif strcmp(Task,'Refocusing') || strcmp(Task,'ColorCoding') 
    If_Speckle=0;
    If_normalization=1;
    If_DF=0;
    If_FF=0;
...
Spectrogram_Y_ROI=[1 Colomn_Read];
%% STFT 1D (PZT Speed Analysis)
if strcmp(Task,'CCDZ')
    If_Refocusing=0;
    If_Color_Coding=0;    %1 for 2D 2 for 3D
    If_LineScan=0;
...
elseif strcmp(Task,'Refocusing')
    If_Refocusing=2;    %1 for 2D 2 for 3D
    If_Color_Coding=0;    %1 for 2D 2 for 3D

    Refocusing_Coef=1;
    Refocusing_XROI=[1 1024];
    Refocusing_YROI=[1 1024];   %11 for EM2
    ...
    Predicted_Central_Wavelength=0.78;
    If_Save_Raw=0;
elseif strcmp(Task,'ColorCoding')
    If_Refocusing=0;    %1 for 2D 2 for 3D
    If_Color_Coding=2;    %1 for B-scan 2 for E-scan
    Refocusing_Coef=1;
    Refocusing_XROI=[1 1024];
...
for QQQ=1:length(folder_list)
    ...
    for p=1:length(file_list)
        ...
        elseif If_Rotate90 == 1
            % read full frame, 然後在裁部分
            fseek(fin, Row*Colomn*2*(Frame_Number_in_File_List(p)-1), 'bof');
            Image_Temp=fread(fin,[Row,Colomn],'uint16');
            Image_Stack(:,:,p)=Image_Temp((Number_of_Row_Skipped+1):(Number_of_Row_Skipped+Colomn_Read),1:Row)';
        end
        fclose('all');
        disp(p);
    end
...
%%
    ...
    Phase_Direction='X';        %Y for EM3, X for EM2
...
%% Refocusing
C=0:0.25:1;%-0.1:0.1:1; %time 1E-9
...
for p=1:length(C)
    ...
    %%
        Cmax=60; %50 60
        Cmin=2;    
        Frame_Thickness=32; %384 128 32
        Start_Frame=176;    %1 128 176
...
%     plot(C,Entropy_Array/max(Entropy_Array));
    ...
    if If_Saptial_Frequency_BP == 1
        ...
        %
            ...
            colormap(gray);
            % ylim([300 400])
            imwrite(Normalized_Image,[Data_Save_Folder sprintf('%s_DAO_C%g_Z%g_Window%g.tif',folder_list(QQQ).name,C_Best,Z0_Pixel,p)],'tiff');
        end
    end
    Reduced_Image_Sum=Reduced_Image_Sum/size(Window_Array,1);
    ...
    colormap(gray);
    % ylim([300 400])
    imwrite(Normalized_Image,[Data_Save_Folder sprintf('%s_DAO_C%g_Z%g_SumWindow.tif',folder_list(QQQ).name,C_Best,Z0_Pixel)],'tiff');
...
%% Color for Depth
if If_Color_Coding ==1  %2 for 3D
    if If_Refocusing >0
        Image_Stack_Complex=Image_Stack_Complex_Corrected_Refocused;
    else
        Image_Stack_Complex=ifft(Image_Stack_FFT,[],3);
        clear Image_Stack_FFT
    end
    %%
    subplot(1,1,1)
    imagesc(TSAutoBrightness(squeeze(TSBinning(mean(abs(Image_Stack_Complex),2),3,16))',0.99,0.05));
    colormap(gray);
%%
    Y_index=8;
    Z_ROI_FFTY=[91*16 400*16];
    subplot(1,1,1)
    imagesc(TSAutoBrightness(squeeze(TSBinning(abs(Image_Stack_Complex(:,Y_index,Z_ROI_FFTY(1):Z_ROI_FFTY(2))),3,16))',0.99,0.05));
%% Median along Y
    Z_ROI_FFTY=[91*16 400*16];

    subplot(2,1,1)
    imagesc(TSAutoBrightness(squeeze(TSBinning(median(abs(Image_Stack_Complex(:,:,Z_ROI_FFTY(1):Z_ROI_FFTY(2))),2),3,16))',0.99,0.05))

    subplot(2,1,2)
    imagesc(TSAutoBrightness(squeeze(TSBinning(mean(abs(Image_Stack_Complex(:,:,Z_ROI_FFTY(1):Z_ROI_FFTY(2))),2),3,16))',0.99,0.05))

    %%
    Y_ROI=[1 11];%[16 35];
    Image_Stack_Complex_FFTY=fft(Image_Stack_Complex,[],2);
    Image_Stack_Abs_FFTY=fft(abs(Image_Stack_Complex(:,Y_ROI(1):Y_ROI(2),:)),[],2);

    %%
    X_ROI_FFTY=[129 896];
    Z_ROI_FFTY=[1001 6120];%[91*16 400*16];
    FFTY_Bin=2;
    Sigma=1;
    ABTH=0.99;
    CTH=0.5;
...
%     subplot(3,1,1)
%     imagesc(TSAutoBrightness(squeeze(TSBinning(abs(Image_Stack_Abs_FFTY(:,1,Z_ROI_FFTY(1):Z_ROI_FFTY(2))),3,16))',0.99,0.1))
%     axis off
    Normalized_Image=TSAutoBrightness(imgaussfilt(squeeze(TSBinning(abs(Image_Stack_Abs_FFTY(X_ROI_FFTY(1):X_ROI_FFTY(2),FFTY_Bin,Z_ROI_FFTY(1):Z_ROI_FFTY(2)))./abs(Image_Stack_Abs_FFTY(X_ROI_FFTY(1):X_ROI_FFTY(2),1,Z_ROI_FFTY(1):Z_ROI_FFTY(2))),3,16)),Sigma)',ABTH,CTH);
    subplot(2,1,1)
    imagesc(Normalized_Image)
    axis off
    imwrite(Normalized_Image,[Data_Save_Folder sprintf('%s_FFTYmap_%gpixel_%gbin.tiff',folder_list(QQQ).name,Y_ROI(2)-Y_ROI(1)+1,FFTY_Bin)],'tiff');

    subplot(2,1,2)
    imagesc(TSAutoBrightness(squeeze(TSBinning(abs(Image_Stack_Complex(X_ROI_FFTY(1):X_ROI_FFTY(2),Y_Bin,Z_ROI_FFTY(1):Z_ROI_FFTY(2))),3,16))',0.99,0.1))
    axis off


    %% Compare Complex and Abs FFTY spectrum (in ROI)
Z_ROI_FFTY=[181 230];
FFTY_Spectrum_Complex=squeeze(mean(mean(abs(Image_Stack_Complex_FFTY(X_ROI_FFTY(1):X_ROI_FFTY(2),:,Z_ROI_FFTY(1):Z_ROI_FFTY(2))),1),3));
FFTY_Spectrum_Abs=squeeze(mean(mean(abs(Image_Stack_Abs_FFTY(X_ROI_FFTY(1):X_ROI_FFTY(2),:,Z_ROI_FFTY(1):Z_ROI_FFTY(2))),1),3));

%plot(FFTY_Spectrum_Complex)
%plot(FFTY_Spectrum_Abs)

  %% spatial freq along Z
  
    If_Complex=0;
    If_YAVE=0;  %if If_YAVE == 1, 表示先YAVE, 再做Z方向之空間頻率分析, 只在分析空間頻率(If_Complex=0)時有用
    
    Xindex_Range=[129 896];
    Yindex_Range=[1 11];
    PreAVE_forSF=1;
    Axial_Window_Size_Pixel=384/PreAVE_forSF;
    Axial_Sampling_Spacing=16/PreAVE_forSF;   %i.e. 此值為1時, overlap = Axial_Window_Size_Pixel - 1, 16洽為0.56micron
    ...
    Image_Intensity=squeeze(TSBinning(mean(abs(Image_Stack_Complex(Xindex_Range(1):Xindex_Range(2),Yindex_Range(1):Yindex_Range(2),Zindex_Range(1):Zindex_Range(2)-WindowOverlap)),2),3,Axial_Sampling_Spacing*PreAVE_forSF))';

    % complex first, 當是complex時, spectrogram會自動輸出完整頻譜
    
    if If_Complex == 0
        if If_YAVE == 0
            Image_Stack_Complex_Virtual_Bscan=TSBinning(abs(reshape(Image_Stack_Complex(Xindex_Range(1):Xindex_Range(2),Yindex_Range(1):Yindex_Range(2),Zindex_Range(1):Zindex_Range(2)),(Xindex_Range(2)-Xindex_Range(1)+1)*(Yindex_Range(2)-Yindex_Range(1)+1),[])),2,PreAVE_forSF);
        elseif If_YAVE == 1
            Image_Stack_Complex_Virtual_Bscan=TSBinning(squeeze(mean(abs(Image_Stack_Complex(Xindex_Range(1):Xindex_Range(2),Yindex_Range(1):Yindex_Range(2),Zindex_Range(1):Zindex_Range(2))),2)),2,PreAVE_forSF);
            Yindex_Range=[1 1];
        end
        Image_Spatial_Spectrogram=zeros(size(Image_Stack_Complex_Virtual_Bscan,1),floor((size(Image_Stack_Complex_Virtual_Bscan,2)-WindowOverlap)/Axial_Sampling_Spacing),Axial_Window_Size_Pixel/2+1);
    elseif If_Complex == 1
        Image_Stack_Complex_Virtual_Bscan=TSBinning(reshape(Image_Stack_Complex(Xindex_Range(1):Xindex_Range(2),Yindex_Range(1):Yindex_Range(2),Zindex_Range(1):Zindex_Range(2)),(Xindex_Range(2)-Xindex_Range(1)+1)*(Yindex_Range(2)-Yindex_Range(1)+1),[]),2,PreAVE_forSF);
        Image_Spatial_Spectrogram=zeros(size(Image_Stack_Complex_Virtual_Bscan,1),floor((size(Image_Stack_Complex_Virtual_Bscan,2)-WindowOverlap)/Axial_Sampling_Spacing),Axial_Window_Size_Pixel);
    end
    for p=1:size(Image_Stack_Complex_Virtual_Bscan,1)
        if If_Complex == 1
            Image_Spatial_Spectrogram(p,:,:)=abs(spectrogram(Image_Stack_Complex_Virtual_Bscan(p,:),Axial_Window_Size_Pixel,WindowOverlap,Axial_Window_Size_Pixel))';  %1st 0 for zero overlapping, nBin for DFT bins, it will outout only half spectrum (512+1)
        elseif If_Complex == 0
            Image_Spatial_Spectrogram(p,:,:)=abs(spectrogram(abs(Image_Stack_Complex_Virtual_Bscan(p,:)),Axial_Window_Size_Pixel,WindowOverlap,Axial_Window_Size_Pixel))';  %1st 0 for zero overlapping, nBin for DFT bins, it will outout only half spectrum (512+1)
        end
            %把spectrum放到dim 3
        disp(p);
    end
    ...
    %% 改看1st bin, 理論上也是DC (但因為現在是complex input, 所以有ambiguity)
    ...
    Image_SF_Temp_DC=squeeze(mean(reshape(Image_Spatial_Spectrogram(:,:,Bin_DC),Xindex_Range(2)-Xindex_Range(1)+1,Yindex_Range(2)-Yindex_Range(1)+1,[]),2))';
    Image_SF_Temp_Norm=imgaussfilt(Image_SF_Temp./Image_SF_Temp_DC,Sigma);
    subplot(2,1,1)
    imagesc(TSAutoBrightness(Image_Intensity,0.985,0.05));
    imwrite(TSAutoBrightness(Image_Intensity,0.985,0.05),[Data_Save_Folder sprintf('%s_Intensity.tiff',folder_list(QQQ).name)],'tiff');
...
%     subplot(3,1,2)
%     plot(squeeze(mean(mean(Image_Spatial_Spectrogram,1),2)))
%     colormap(gray);
    %% Peak Frequency Calculation
    [Value PeakFreq_Index_Stack]=max(Image_Spatial_Spectrogram,[],3);
    Image_PeakFrequency=squeeze(mean(reshape(PeakFreq_Index_Stack,Xindex_Range(2)-Xindex_Range(1)+1,Yindex_Range(2)-Yindex_Range(1)+1,[]),2))';
    PeakFrequency_Profile=mean(Image_PeakFrequency,2);
    imagesc(Image_PeakFrequency)
    plot(PeakFrequency_Profile)
    ylim([60 80])
    saveas(gcf,[Data_Save_Folder sprintf('%s_2D Spectrum Images.png',folder_list(QQQ).name)]);
%%
    Z_offset=12;
    V_max=40;    %40 60
    V_min=5;
    Color_max=1.21;    %80 50
    Color_min=0.23;      %2  10
    ...
    Sat=1;
    HSV_Image=ones(size(Image_SF_Temp_Norm,1),size(Image_SF_Temp_Norm,2),3).*Sat;
    Color_Map=(Image_SF_Temp_Norm-Color_min)/(Color_max-Color_min);
    Color_Map(Color_Map>1)=1;
    Color_Map(Color_Map<0)=0;
    Color_Map=circshift(Color_Map,Z_offset,1);
    histogram(Color_Map);
    imwrite(Color_Map,[Data_Save_Folder sprintf('%s_FFTZmap_%gpixel_PreAVE%g.tiff',folder_list(QQQ).name,Zindex_Range(2)-Zindex_Range(1)+1,PreAVE_forSF)],'tiff');
    ...
    elseif If_YAVE == 1
        imwrite(RGB_Image,[Data_Save_Folder sprintf('%s_Corrected_Start%g_End%g_Color_YAVE.tif',folder_list(QQQ).name,File_Reading_Range(1),File_Reading_Range(2))],'tiff');
    end
elseif If_Color_Coding ==2  %2 for 3D
    if If_Refocusing >0
        Image_Stack_Complex=Image_Stack_Complex_Corrected_Refocused;
    else
        Image_Stack_Complex=ifft(Image_Stack_FFT,[],3);
    ...
    Start_Frame_Fine=129;%1
    Frame_Thickness_Fine=256;%384
        
    Blur_Sigma=2;
    PREAVE_forSF=384/128;%3
    Image_Intensity_Fine=mean(abs(Image_Stack_Complex(:,:,Start_Frame_Fine:Frame_Thickness_Fine)),3);

    Image_Intensity=mean(abs(Image_Stack_Complex(:,:,Start_Frame:End_Frame)),3);
    Image_Intensity_Max=max(TSBinning(abs(Image_Stack_Complex(:,:,Start_Frame:End_Frame)),3,PREAVE_forSF),[],3);
    Temp_Stack=TSBinning(abs(Image_Stack_Complex(:,:,Start_Frame:End_Frame)),3,PREAVE_forSF);
    Temp_Stack=Temp_Stack-repmat(mean(Temp_Stack,3),[1 1 Frame_Thickness/PREAVE_forSF]);
    Image_Axial_Spectrum=abs(fft(Temp_Stack,[],3))./repmat(Image_Intensity,[1 1 Frame_Thickness/PREAVE_forSF]);
...
Image_Intensity_Fine=Image_Intensity_Fine';
...
imwrite(Image_Axial_SpeFreq_Blur_Norm,[Data_Save_Folder sprintf('%s_Axial_SpeFreq_Blur_PreAVE%g.tiff',folder_list(QQQ).name,PREAVE_forSF)],'tiff');

    imwrite(TSAutoBrightness(Image_Intensity,0.999,0.1),[Data_Save_Folder sprintf('%s_Intensity.tiff',folder_list(QQQ).name)],'tiff');
    imwrite(TSAutoBrightness(Image_Intensity_Fine,0.995,0.05),[Data_Save_Folder sprintf('%s_Intensity_Fine.tiff',folder_list(QQQ).name)],'tiff');
...
%% hsv to rgb
    V_max=40;    %40 60
    V_min=5;
    Color_max=0.9091;    %80 50
    Color_min=0.0227;      %2  10
    Color_Index_Center=0.21;%0.21
    Color_Index_Span=0.5;%0.5
    Color_Index_Range=[Color_Index_Center-Color_Index_Span/2 Color_Index_Center+Color_Index_Span/2];
    Sat=1;
    HSV_Image=ones(size(Image_Axial_SpeFreq_Blur_Norm,1),size(Image_Axial_SpeFreq_Blur_Norm,2),3).*Sat;
    HSV_Image_Fine=ones(size(Image_Axial_SpeFreq_Blur_Norm,1),size(Image_Axial_SpeFreq_Blur_Norm,2),3).*Sat;
    Color_Map=(Image_Axial_SpeFreq_Blur_Norm-Color_min)/(Color_max-Color_min);
    Color_Map(Color_Map>1)=1;
    Color_Map(Color_Map<0)=0;
    histogram(Color_Map);
    %
    ...
    HSV_Image(:,:,1)=Color_Map;
    HSV_Image(:,:,3)=(Image_Intensity-V_min)/(V_max-V_min);
    HSV_Image_Fine(:,:,1)=Color_Map;    
    HSV_Image_Fine(:,:,3)=(Image_Intensity_Fine-V_min)/(V_max-V_min);
    RGB_Image=hsv2rgb(HSV_Image);
    RGB_Image_Fine=hsv2rgb(HSV_Image_Fine);
    ...
    imwrite(RGB_Image,[Data_Save_Folder sprintf('%s_Corrected_Start%g_End%g_PREAVE%g_Color.tif',folder_list(QQQ).name,File_Reading_Range(1),File_Reading_Range(2),PREAVE_forSF)],'tiff');
    imwrite(RGB_Image_Fine,[Data_Save_Folder sprintf('%s_Corrected_Start%g_End%g_PREAVE%g_Color_Fine.tif',folder_list(QQQ).name,File_Reading_Range(1),File_Reading_Range(2),PREAVE_forSF)],'tiff');
%%
% 
%                        KX_Grid=repmat(reshape([1:size(Image_Stack_FFTX,1)],[size(Image_Stack_FFTX,1) 1 1]),[1 size(Image_Stack_FFTX,2) size(Image_Stack_FFTX,3)]);
%                        KX_Grid((round(size(Image_Stack_FFTX,1)/2)+1):end,:,:)=KX_Grid(round(size(Image_Stack_FFTX,1)/2):-1:1,:,:);
