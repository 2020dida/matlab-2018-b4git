...
Dia(Dia==0)=1;
%% Scatterer Plot
scatter(X_Attribute,Y_Attribute,50,log10(C_Attribute),'filled','MarkerFaceAlpha',.8);
hold on
scatter(X_Mean,Y_Mean,Dia*500,log10(C_Mean),'filled','MarkerFaceAlpha',.8);
hold off
title(sprintf('Mode Diameter=%gmicron',Mode_Diameter),'Color','b')
%set(gca,'yscale','log')
colormap(jet)
caxis([4 6])
colorbar
...
dx = 0; 
dy = 0;
%textfit(Radiance_Norm_dB+dx, Related_BW+dy, Lebal,0.5,0,'Color',[0.5 0.5 0.5],'FontSize',12,'HorizontalAlignment','center');
