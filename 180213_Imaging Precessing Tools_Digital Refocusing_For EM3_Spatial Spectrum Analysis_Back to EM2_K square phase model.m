...
%% Phase Model: exp(i*C*(Z-Z0)*(KX-KX0)^2), 3 const model
    ...
    for p=1:length(C)
        Image_Stack_FFTX_Image_Corrected=abs(ifft((Image_Stack_FFTX(:,:,(Z_Test:Z_Test+4*N)).*exp(i*C(p)*(1E-9).*(Z_Grid(:,:,(Z_Test:Z_Test+4*N))-Z0).*((KX_Grid(:,:,(Z_Test:Z_Test+4*N))-KX0).^2))),[],1));
    ...
    histogram(Image_Stack_FFTX_Image_Corrected(:))
...
%     plot(C,Entropy_Array/max(Entropy_Array));
    ...
    C_Best=C(index_Best);
    %
    Image_Stack_FFTX_Corrected=abs(ifft((Image_Stack_FFTX.*exp(i*C_Best*(1E-9).*(Z_Grid-Z0).*((KX_Grid-KX0).^2))),[],1));
    Reduced_Image_Corrected=squeeze(TSBinning(mean(Image_Stack_FFTX_Corrected,2),3,N*4))'; %2 for PostAVE*PreAVE
%
    Z_Test=220*N*4
    Normalized_Sharpness_atZTest=sum(abs(diff(Reduced_Image_Corrected(Z_Test/N/4,:),1,2)))/mean(Reduced_Image_Corrected(Z_Test/N/4,:))
%
    % Auto-brightness
    ...
    colormap(gray);
    %ylim([300 400])
    imwrite(Normalized_Image,[Data_Save_Folder sprintf('%s_DAO_C%g.tif',folder_list(QQQ).name,C_Best)],'tiff');
