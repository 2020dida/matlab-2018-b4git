clear all
%%
    Lateral_Scanning_Speed=1.92;   %mm/sec
...
%%
root_folder_path='C:\TuanShu\180104_Pork Test\';
last_folder_name='1.92mmsec';
Number=[1];
%%
...
if If_LineScan == 1
    ...
    if If_Demodulation == 1
        ...
        Frame_Spacing_Lateral_Pixel=round(Lateral_Scanning_Speed_Pixel/(Frame_Rate/N));
    else
        Image_Stack_Used=Image_Stack;
        Lateral_Scanning_Speed_Pixel=Lateral_Scanning_Speed*1000/Lateral_SR;
        Frame_Spacing_Lateral_Pixel=round(Lateral_Scanning_Speed_Pixel/Frame_Rate);
    end
        Stitched_Image=zeros(size(Image_Stack_Used,1),size(Image_Stack_Used,3)*Frame_Spacing_Lateral_Pixel);
    ...
    %if Overlapping_Mode ==1
        ...
        for p=1:size(Image_Stack_Used,3)
            if Overlapping_Mode ==1
                ...
                Stitched_Image(:,((p-1)*Frame_Spacing_Lateral_Pixel+1):(p*Frame_Spacing_Lateral_Pixel))=Image_Stack_Used(:,Y_Pixel_Phase:(Y_Pixel_Phase+Frame_Spacing_Lateral_Pixel-1),p);
            elseif Overlapping_Mode ==2
                if ((p-1)*Frame_Spacing_Lateral_Pixel+size(Image_Stack_Used,2))<=size(Stitched_Image,2)
                    if Order == 1
                        Stitched_Image(:,((p-1)*Frame_Spacing_Lateral_Pixel+1):((p-1)*Frame_Spacing_Lateral_Pixel+size(Image_Stack_Used,2)))=Stitched_Image(:,((p-1)*Frame_Spacing_Lateral_Pixel+1):((p-1)*Frame_Spacing_Lateral_Pixel+size(Image_Stack_Used,2)))+Image_Stack_Used(:,end:-1:1,p);
                    elseif Order == 2
                        Stitched_Image(:,((p-1)*Frame_Spacing_Lateral_Pixel+1):((p-1)*Frame_Spacing_Lateral_Pixel+size(Image_Stack_Used,2)))=Stitched_Image(:,((p-1)*Frame_Spacing_Lateral_Pixel+1):((p-1)*Frame_Spacing_Lateral_Pixel+size(Image_Stack_Used,2)))+Image_Stack_Used(:,:,p);
...
%                 Stitched_Image=Stitched_Image+circshift(squeeze(Image_Stack(:,p,:)),-1*Frame_Spacing_Lateral_Pixel*(p-1),2);
%             end
%         elseif Order == 2
%             for p=1:size(Image_Stack,2)
%                 Stitched_Image=Stitched_Image+circshift(squeeze(Image_Stack(:,p,:)),1*Frame_Spacing_Lateral_Pixel*(p-1),2);
...
%     end
    ...
    %
    if Overlapping_Mode ==1
        imwrite(Stitched_Image,[Data_Save_Folder '\' folder_list(QQQ).name sprintf('_Line_Scan_Decimated_Y%g.png',Y_Pixel_Phase)],'png');
    elseif Overlapping_Mode ==2
        imwrite(Stitched_Image,[Data_Save_Folder '\' folder_list(QQQ).name sprintf('_Line_Scan_Averaged_O%g.png',Order)],'png');
    end
