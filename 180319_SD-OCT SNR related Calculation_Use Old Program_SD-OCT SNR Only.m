...
for q=1:length(Reference_R)
    Reference_Power=SLD_Power*BS_Ratio*(1-BS_Ratio)*(Reference_SinglePass_CE^2)*Reference_R(q);
    Ratio_R=BS_Ratio*(1-BS_Ratio)*(Reference_SinglePass_CE^2)*Reference_R(q);

    Sample_Power=SLD_Power*(1-BS_Ratio)*BS_Ratio*(Sample_SinglePass_CE^2)*Sample_R;
    ...
    %%
    Number_of_Electron_per_Line=Spectrometer_CE*(Reference_Power+Sample_Power)*1E-3.*Exp_Time/(Plank*3E8/(Center_Wavelengh.*1E-6));
    ...
    IE(q)=min(IE_Max,2*((Sample_Power*Reference_Power)^0.5)/(Sample_Power+Reference_Power));
...
% Rr=1;
% FWC=400000;
% Signal=1/N*((GratingE*QE*exp_Time*P0/h/v0)^2)*gama_R*gama_S*Rr;
% Shot=1/N*(GratingE*QE*exp_Time*P0/h/v0)*gama_R*Rr;
% SNR_shot=10*log10(Signal./Shot)
% %% Theoritical SNR (with FWC constrain)
% GratingE=0.19;
% QE=0.4;
% exp_Time=1E-3;
% h=Plank;
...
% Rr=1;
% FWC=400000;
% Signal=1/N*((GratingE*QE*exp_Time*min(P0/h/v0,FWC*N))^2)*gama_R*gama_S*Rr;
% Shot=1/N*(GratingE*QE*exp_Time*min(P0/h/v0,FWC*N))*gama_R*Rr;
% SNR_shot=10*log10(Signal./Shot)
% %%
% 10*log10(0.6*(0.75E-3)/(h*v0/50000))
%% Simple Est
...
ylabel('Signal-to-Noise Ratio (dB, shot-noise-limited)')
dlmwrite([Data_Save_Folder 'SNRvsRr.txt'],[Reference_R'*100 SNR_SDOCT'],'delimiter','\t','newline','pc','precision', '%.6f','-append');

%%

    subplot(2,1,1)
    plot(Reference_R*100,Sturation_Level*100,'r','LineWidth',3);
    xlabel('Reference Arm Reflectance (%)')
    ylabel('Saturation Level(%)')    
    
    subplot(2,1,2)
    plot(Reference_R*100,IE*100,'r','LineWidth',3);
    xlabel('Reference Arm Reflectance (%)')
