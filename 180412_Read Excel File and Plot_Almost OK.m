...
%%
Job='RadianceLimited';        %Radiance, RadianceNoClass, RadianceLimited
Mode_NA=0.14;       %0.14 0.22 0.5
Mode_Diameter=4.6;  %4.6 106 5000
...
for p=1:Number_of_Line
    ...
    Lebal{p}=['\fontsize{10}' Model{p} '\fontsize{6}' Manufacturer{p}];
end
% Data Exclusion
% Index_Excluded=[];
% Mask=Index;               %4
...
C_Attribute=Price;
%% Data Grouping
Type_of_Interest={'Crystal Fiber' 'Fiber-coupled LED (visible)' 'Fiber-coupled LED (IR)' 'Lamp' 'Single SLD' 'Multiple SLD' 'Supercontinuum' 'Plasma'};
...
Dia(Dia==0)=1;
%% Scatterer Plot
if strcmp(Job,'RadianceNoClass') 
    scatter(X_Attribute,Y_Attribute,200,log10(C_Attribute),'filled','MarkerFaceAlpha',1);
    textfit(Radiance_Norm_dB, Related_BW, Lebal,0,0,'Color',[1 1 1],'FontSize',14,'HorizontalAlignment','center','FontWeight','bold');
else
    ...
    hold off
    textfit(X_Mean, Y_Mean, Type_of_Interest,0,0,'Color',[1 1 1],'FontSize',10,'HorizontalAlignment','center','FontWeight','bold');
...
if strcmp(Job,'Radiance')
    title('Different Types of Light Source','Color','b')
elseif strcmp(Job,'RadianceNoClass') 
    title('Commercially Available Light Sources','Color','b')
elseif strcmp(Job,'RadianceLimited')
...
if strcmp(Job,'Radiance')
    saveas(gcf,[filepath '\' 'Radiance.png']);
elseif strcmp(Job,'RadianceNoClass') 
    saveas(gcf,[filepath '\' 'Radiance_NoClass.png']);
elseif strcmp(Job,'RadianceLimited')
    saveas(gcf,[filepath '\' sprintf('RadianceLimited_Dia%g_NA%g.png',Mode_Diameter,Mode_NA)]);
end
