...
elseif If_Color_Coding ==2  %2 for 3D
    ...
    Blur_Sigma=2;
    Axial_Smooth_Before_Process=3;%3
    ...
    Image_Intensity_Max=max(TSBinning(abs(Image_Stack_Complex(:,:,Start_Frame:End_Frame)),3,Axial_Smooth_Before_Process),[],3);
    Temp_Stack=TSBinning(abs(Image_Stack_Complex(:,:,Start_Frame:End_Frame)),3,Axial_Smooth_Before_Process);
...
%% hsv to rgb
    ...
    HSV_Image_Fine=ones(size(Image_Axial_SpeFreq_Blur,1),size(Image_Axial_SpeFreq_Blur,2),3).*Sat;
    ...
    imwrite(RGB_Image,[Data_Save_Folder sprintf('%s_Corrected_Start%g_End%g_Color.tif',folder_list(QQQ).name,File_Reading_Range(1),File_Reading_Range(2))],'tiff');
    imwrite(RGB_Image_Fine,[Data_Save_Folder sprintf('%s_Corrected_Start%g_End%g_Color_Fine.tif',folder_list(QQQ).name,File_Reading_Range(1),File_Reading_Range(2))],'tiff');
%%
% 
%                        KX_Grid=repmat(reshape([1:size(Image_Stack_FFTX,1)],[size(Image_Stack_FFTX,1) 1 1]),[1 size(Image_Stack_FFTX,2) size(Image_Stack_FFTX,3)]);
%                        KX_Grid((round(size(Image_Stack_FFTX,1)/2)+1):end,:,:)=KX_Grid(round(size(Image_Stack_FFTX,1)/2):-1:1,:,:);
