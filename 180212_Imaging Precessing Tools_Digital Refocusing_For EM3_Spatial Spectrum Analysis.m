...
for QQQ=1:length(folder_list)
    ...
    %
    File_Reading_Range=[7000 7500];%7500];
...
%     plot(squeeze(real(Image_Stack_Corrected(PlotX,PlotY,:))));
    ...
    R=0.03125;
    FrameN=72;
    TEMPTEMP=abs(fft((((Image_Stack_FFTX(:,22,FrameN)))),[],1));
    plot(TEMPTEMP);
...
elseif If_Refocusing == 2
    ...
    %%
    Refocusing_ZROI_EmodeOnly=[1 50]+200;
%     PlotX=100;
%     PlotY=1;
%     plot(squeeze(real(Image_Stack_FFT(PlotX,PlotY,:))));
%     xlim([Index_SP Index_LP]);
    ...
    %TEMPTEMP=abs((fft((((Image_Stack_Complex(:,:,FrameN+Refocusing_ZROI_EmodeOnly(1))))),[],2)));
