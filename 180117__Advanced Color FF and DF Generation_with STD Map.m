...
Frame=length(File_list);
   ...
   %% File Loading
Image_R_Stack=zeros(Colomn,Row,Frame);
Image_G_Stack=zeros(Colomn,Row,Frame);
Image_B_Stack=zeros(Colomn,Row,Frame);
...
for p=1:length(File_list)
    ...
    Image_R_Stack(:,:,p)=double(Image_Temp(:,:,1));
    Image_G_Stack(:,:,p)=double(Image_Temp(:,:,2));
    Image_B_Stack(:,:,p)=double(Image_Temp(:,:,3));
...
Image_R_STD=std(Image_R_Stack,[],3);
Image_G_STD=std(Image_G_Stack,[],3);
