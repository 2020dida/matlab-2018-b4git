...
%%
If_Rotate90=0;  %slow but works 1
Row=1024;
Colomn=1024;
Colomn_Read=1024;%50
Number_of_Row_Skipped=0;  %485
...
elseif strcmp(Task,'ColorCoding')
    ...
    If_Color_Coding=2;    %1 for 2D 2 for 3D
    Refocusing_Coef=1;
    Refocusing_XROI=[1 1024];
...
plot(FFTY_Spectrum_Abs)
  ...
  %% spatial freq along Z
    ...
    Image_Stack_Complex_Virtual_Bscan=reshape(Image_Stack_Complex(Xindex_Range(1):Xindex_Range(2),Yindex_Range(1):Yindex_Range(2),Zindex_Range(1):Zindex_Range(2)),(Xindex_Range(2)-Xindex_Range(1)+1)*(Yindex_Range(2)-Yindex_Range(1)+1),[]);
    if If_Complex == 0
    ...
    imagesc(TSAutoBrightness(diff(Image_SF_Temp_Norm,1,2),ABTH,CminR));
    subplot(3,1,3)
...
elseif If_Color_Coding ==2  %2 for 3D
    ...
    Start_Frame=1;%1
    Frame_Thickness=280;%384
    End_Frame=Start_Frame+Frame_Thickness-1;
    Blur_Sigma=2;
    Axial_Smooth_Before_Process=14;%3
...
ABTH=0.99;
CTH=0.62;
Normalized_Image=TSAutoBrightness(imgaussfilt(squeeze(abs(Image_Axial_Spectrum(:,:,Nindex))),Sigma)',ABTH,CTH);
imagesc(Normalized_Image)
imwrite(Normalized_Image,[Data_Save_Folder sprintf('%s_FFTYmap_%gpixel_%gbin.tiff',folder_list(QQQ).name,Frame_Thickness,Nindex)],'tiff');
axis equal
axis off
%%
subplot(1,1,1)
