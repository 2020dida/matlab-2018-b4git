...
%% 
If_Plot_with_TXT=1;
TXT_Path='C:\TuanShu\180517_Image Normalization for Comparison\MySystem\Plan N 20X\Processed Data\Vertical.txt';
if If_Plot_with_TXT ==1
        TXT=dlmread(TXT_Path);
end
%%
...
if If_Plot_MTF == 1
    clf
    if If_Plot_with_TXT ==0
        plot(F,MTF*100,'LineWidth',4);
    else
        plot(F,MTF*100,TXT(:,1),TXT(:,2)*100,'LineWidth',4);
        legend('Diffraction-limited','Measured','Measured-corrected');
    end
    xlim([0 1200/Spatial_Period*2]);
    ylim([0 100])
    set(gca, 'FontSize', 14)
    grid on
