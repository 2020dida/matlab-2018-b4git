...
last_folder_name='180212_EM2 Data for Processing (OBL)\';
Number=[1];
%%
...
%     end
    ...
    if (If_Bandpass==1)
        ...
        elseif If_Refocusing == 1
            ...
            Image_Stack_Refocusing_ROI=Image_Stack_Refocusing_ROI-repmat(mean(Image_Stack_Refocusing_ROI,3),[1 1 size(Image_Stack_Refocusing_ROI,3)]);
        ...
        %%
        imagesc(Image_Stack_Refocusing_ROI(:,:,141));
...
%     PlotX=100;
%     PlotY=1;
%     plot(squeeze(real(Image_Stack_FFT(PlotX,PlotY,:))));
%     xlim([Index_SP Index_LP]);
    % XFFT
    Image_Stack_FFTX=fft(ifft(Image_Stack_FFT,[],3),[],1);
    %Image_Stack_FFTX((round(size(Image_Stack_FFTX,1)/2)+1):end,:,:)=0;
    %plot(squeeze(real(Image_Stack_FFTX(PlotX,PlotY,:))));
    Z_Grid=repmat(reshape((1:size(Image_Stack_FFTX,3))*Predicted_Central_Wavelength/2/RI/N/ave_factor/Refocusing_ZeroPaddingRatio,[1 1 size(Image_Stack_FFTX,3)]),[size(Image_Stack_FFTX,1) size(Image_Stack_FFTX,2) 1]);                    %micron
    X_Spatial_Frequency_Resolution=1/(Lateral_SR*(Refocusing_XROI(2)-Refocusing_XROI(1)+1));
    KX_Grid=(repmat(reshape(2*pi*(X_Spatial_Frequency_Resolution:X_Spatial_Frequency_Resolution:(size(Image_Stack_FFTX,1)*X_Spatial_Frequency_Resolution)),[size(Image_Stack_FFTX,1) 1 1]),[1 size(Image_Stack_FFTX,2) size(Image_Stack_FFTX,3)]));
    KX_Grid((round(size(Image_Stack_FFTX,1)/2)+1):end,:,:)=KX_Grid(round(size(Image_Stack_FFTX,1)/2):-1:1,:,:);
    %% Try directly delecting the phase
    If_Correct=1;
    if If_Correct ==1
        Image_Stack_FFTX_Corrected=abs(ifft(abs(Image_Stack_FFTX),[],1));
    else
        Image_Stack_FFTX_Corrected=abs(ifft((Image_Stack_FFTX),[],1));
    end
    Reduced_Image_Corrected=squeeze(TSBinning(mean(Image_Stack_Corrected,2),3,N*4))'; %2 for PostAVE*PreAVE

%     Refocusing_Coef=0;%0.05;
%     Z0=0;
%     Phase_Correction_Term=exp(i*Refocusing_Coef*Predicted_Central_Wavelength/4*pi/RI*(Z_Grid-Z0).*(KX_Grid.^2));
%     %ZKX=Z_Grid.*(KX_Grid.^2);
%     %imagesc(real(squeeze(KX_Grid(:,1,:))));
%     %plot(squeeze(real(Z_Grid(512,3,:))))
% 
%     %plot(squeeze(real(Phase_Correction_Term(512,3,:))))
%     Image_Stack_FFTX_Corrected=Image_Stack_FFTX.*Phase_Correction_Term;
%     Image_Stack_Corrected=abs(ifft(Image_Stack_FFTX_Corrected,[],1));
%     Reduced_Image_Corrected=squeeze(TSBinning(mean(Image_Stack_Corrected,2),3,N*4))'; %2 for PostAVE*PreAVE
%     subplot(1,1,1)
%     imagesc(Reduced_Image_Corrected);
%     colormap(gray);
%     subplot(2,1,1)
%     plot(squeeze(real(Image_Stack_FFTX(PlotX,PlotY,:))));
%     subplot(2,1,2)
%     plot(squeeze(real(Image_Stack_Corrected(PlotX,PlotY,:))));
    % Auto-brightness
    ...
    R=0.03125;
    FrameN=6000;
    TEMPTEMP=unwrap(angle(Image_Stack_FFTX(:,6,FrameN)));
    plot(TEMPTEMP);
...
elseif If_Refocusing == 2
    Image_Stack_Complex=ifft(Image_Stack_FFT,[],3);
    clear Image_Stack_FFT
    %%
    Refocusing_ZROI_EmodeOnly=[1 50]+400;
%     PlotX=100;
%     PlotY=1;
%     plot(squeeze(real(Image_Stack_FFT(PlotX,PlotY,:))));
%     xlim([Index_SP Index_LP]);
    % XFFT
    Image_Stack_FFTXY=fft(fft(Image_Stack_Complex(:,:,Refocusing_ZROI_EmodeOnly(1):Refocusing_ZROI_EmodeOnly(2)),[],1),[],2);
    %Image_Stack_FFTX((round(size(Image_Stack_FFTX,1)/2)+1):end,:,:)=0;
    %plot(squeeze(real(Image_Stack_FFTX(PlotX,PlotY,:))));
    Z_Grid=repmat(reshape((1:size(Image_Stack_FFTXY,3))*Predicted_Central_Wavelength/2/RI/N/ave_factor/Refocusing_ZeroPaddingRatio,[1 1 size(Image_Stack_FFTXY,3)]),[size(Image_Stack_FFTXY,1) size(Image_Stack_FFTXY,2) 1]);                    %micron
    XY_Spatial_Frequency_Resolution=1/(Lateral_SR*(Refocusing_XROI(2)-Refocusing_XROI(1)+1));
    
    KX_Grid=(repmat(reshape(2*pi*(XY_Spatial_Frequency_Resolution:XY_Spatial_Frequency_Resolution:(size(Image_Stack_FFTXY,1)*XY_Spatial_Frequency_Resolution)),[size(Image_Stack_FFTXY,1) 1 1]),[1 size(Image_Stack_FFTXY,2) size(Image_Stack_FFTXY,3)]));
    KX_Grid((round(size(Image_Stack_FFTXY,1)/2)+1):end,:,:)=KX_Grid(round(size(Image_Stack_FFTXY,1)/2):-1:1,:,:);
    KY_Grid=(repmat(reshape(2*pi*(XY_Spatial_Frequency_Resolution:XY_Spatial_Frequency_Resolution:(size(Image_Stack_FFTXY,1)*XY_Spatial_Frequency_Resolution)),[1 size(Image_Stack_FFTXY,2) 1]),[size(Image_Stack_FFTXY,1) 1 size(Image_Stack_FFTXY,3)]));
    KY_Grid(:,(round(size(Image_Stack_FFTXY,2)/2)+1):end,:)=KY_Grid(:,round(size(Image_Stack_FFTXY,2)/2):-1:1,:);
   
    %%
    R=1;
    FrameN=9182;
    TEMPTEMP=abs(fft(fft((((Image_Stack_Complex(:,:,FrameN+Refocusing_ZROI_EmodeOnly(1))))),[],1),[],2));
    imagesc(TEMPTEMP);
...
%     imagesc(real(fftshift(fftshift(Image_Stack_FFTXY(:,:,FrameN),1),2)));
    ...
    %%
    Refocusing_Coef=0.65;
    Z0=0;
    Phase_Correction_Term=exp(i*Refocusing_Coef*Predicted_Central_Wavelength/4*pi/RI*(Z_Grid-Z0).*((KX_Grid.^2)+(KY_Grid.^2)));
    %ZKX=Z_Grid.*(KX_Grid.^2);
    %imagesc(real(squeeze(KX_Grid(:,1,:))));
    %plot(squeeze(real(Z_Grid(512,3,:))))

    %plot(squeeze(real(Phase_Correction_Term(512,3,:))))
    Image_Stack_FFTXY_Corrected=Image_Stack_FFTXY.*Phase_Correction_Term;
    Image_Stack_Corrected=abs(ifft(ifft(Image_Stack_FFTXY_Corrected,[],2),[],1));
    E_Image_Corrected=mean(Image_Stack_Corrected,3); %2 for PostAVE*PreAVE
%     subplot(1,1,1)
%     imagesc(Reduced_Image_Corrected);
%     colormap(gray);
%     subplot(2,1,1)
...
%     plot(squeeze(real(Image_Stack_Corrected(PlotX,PlotY,:))));
    ...
    Auto_Brightness_TH=0.995;   %%0.995
    nbin=250;
    Histogram=hist(E_Image_Corrected(:),nbin);
    Total=sum(Histogram);
    Int_Hist = cumsum(Histogram)/Total;

    C_max=find(Int_Hist>Auto_Brightness_TH,1,'first')/nbin*max(E_Image_Corrected(:));
    C_min=C_max*0.1;            %%0.1
    Normalized_Image=(E_Image_Corrected-C_min)/(C_max-C_min);
    Normalized_Image(Normalized_Image<0)=0;
    Normalized_Image(Normalized_Image>1)=1;
    %
    
%     subplot(1,2,1)
%     imagesc(mean(abs(Image_Stack_Complex(:,:,Refocusing_ZROI_EmodeOnly(1):Refocusing_ZROI_EmodeOnly(2))),3));
%     colormap(gray);
%     axis equal
