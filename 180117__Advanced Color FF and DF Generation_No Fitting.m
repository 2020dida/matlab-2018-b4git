...
%%
NN=420;
Line=[Image_R(NN,:)/max(Image_R(NN,:))]';
X=[1:length(Line)]';
plot(Image_R(NN,:)/max(Image_R(NN,:)));
Model=fittype(@ (a,b,c,d,x) (a*exp(-((x-c)/b).^2)+d*x));
Fit_Line=fit(X,Line,Model,'StartPoint',[1 500 500 -1]);

Fitted_Line=Fit_Line.a*exp(-((X-Fit_Line.c)/Fit_Line.b).^2)+Fit_Line.d*X;

plot(X,Fitted_Line,X,Line);
...
axis equal
axis off
Stray_Pattern_Position_X=[360 790];
Stray_Pattern_Position_Y=[410 520];

Image_R_Temp=Image_R;
Image_G_Temp=Image_G;
Image_B_Temp=Image_B;
Image_R_Temp(Stray_Pattern_Position_Y(1):Stray_Pattern_Position_Y(2),Stray_Pattern_Position_X(1):Stray_Pattern_Position_X(2))=0;
Image_G_Temp(Stray_Pattern_Position_Y(1):Stray_Pattern_Position_Y(2),Stray_Pattern_Position_X(1):Stray_Pattern_Position_X(2))=0;
Image_B_Temp(Stray_Pattern_Position_Y(1):Stray_Pattern_Position_Y(2),Stray_Pattern_Position_X(1):Stray_Pattern_Position_X(2))=0;

Image_R_X_Max=max(Image_R_Temp((X_Grid<Stray_Pattern_Position_X(1))|(X_Grid>Stray_Pattern_Position_X(2))),[],2);
Image_R_Yonly=Image_R_Temp./repmat(Image_R_X_Max,[1 size(Image_R_Temp,2)]);
