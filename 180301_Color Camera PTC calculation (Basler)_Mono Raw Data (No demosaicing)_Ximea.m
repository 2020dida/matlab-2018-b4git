...
%%
Known_Frame_Number_per_File=500;
Max_Number_of_Frame=499;
X_ROI=[869 1068];
Y_ROI=[751 950];
Number=[];%9
Set='Ximea';
Dara_Reading_Mode='Stack';   %Single Frames, Stack
...
if strcmp(Set,'Basler')
    root_folder_path='C:\TuanShu\180118_Bayer Disabled Intensity vs Gain\';
    last_folder_name='Separate 2Frames';
    Row=2448;
    Colomn=2048;
    IfColor=1;
elseif strcmp(Set,'Ximea')
    root_folder_path='C:\TuanShu\';
    last_folder_name='180301_Ximea FWC Test_Full Frame_165fps_RAW8';
    Row=1936;
    Colomn=1216;
    IfColor=1;  
...
for QQQ=1:length(folder_list)
    ...
    for p=1:length(file_list_ori)
        ...
        if isempty(Known_Frame_Number_per_File)==0
            Frame_Number_Calculated_Based_on_Size=Known_Frame_Number_per_File;
        end
    ...
    for p=1:Frame
        ...
        if  Frame_Number_Calculated_Based_on_Size>1.1
            Image_Temp=double(imread(file_path,'tiff','Index',mod(Frame_Number_in_File_List(p),Frame_Number_Calculated_Based_on_Size)+1));
        ...
        elseif Frame_Number_Calculated_Based_on_Size<1.1
...
It_Array=[100 200 400 600 800 1000 1200 1400 1600 1800 2000 2500 3000 3500 4000 4500 5000 5500 6000 6500 7000 7500 8000 8500 9000 10000 11000 12000 14000]/1000;
%It_Array=[100 200 400 600];
...
for p=1:3
    ...
    ylim([0 256]);
    %xlim([0 256]);
    Slope=(Mean_Array(6)-Mean_Array(1))/(It_Array(6)-It_Array(1));
    if p ==1
        saveas(gcf,[Data_Save_Folder last_folder_name sprintf('_Method_%g_Linearity_R_Slope%g',Noise_Removal_Method,Slope) '.png']);
        dlmwrite([Data_Save_Folder last_folder_name sprintf('_Method_%g_Linearity_R_Slope%g',Noise_Removal_Method,Slope) '.txt'],[It_Array' Mean_Array] ,'delimiter','\t','newline','pc','precision', '%.6f','-append');
