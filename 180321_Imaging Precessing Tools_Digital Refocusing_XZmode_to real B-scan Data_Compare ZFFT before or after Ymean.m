...
%     axis off
    Normalized_Image=TSAutoBrightness(imgaussfilt(squeeze(TSBinning(abs(Image_Stack_Abs_FFTY(:,FFTY_Bin,Z_ROI_FFTY(1):Z_ROI_FFTY(2)))./abs(Image_Stack_Abs_FFTY(:,1,Z_ROI_FFTY(1):Z_ROI_FFTY(2))),3,16)),Sigma)',ABTH,CTH);
    subplot(2,1,1)
    imagesc(Normalized_Image)
    axis off
    ...
    subplot(2,1,2)
    imagesc(TSAutoBrightness(squeeze(TSBinning(abs(Image_Stack_Complex(:,Y_Bin,Z_ROI_FFTY(1):Z_ROI_FFTY(2))),3,16))',0.99,0.1))
    axis off
...
plot(FFTY_Spectrum_Abs)
  ...
  %% spatial freq along Z
    ...
    Yindex_Range=[1 11];
    PreAVE=1;
    Axial_Window_Size_Pixel=384/PreAVE;
    Axial_Sampling_Spacing=16/PreAVE;   %i.e. 此值為1時, overlap = Axial_Window_Size_Pixel - 1, 16洽為0.56micron
    ...
    Image_Intensity=squeeze(TSBinning(mean(abs(Image_Stack_Complex(Xindex_Range(1):Xindex_Range(2),Yindex_Range(1):Yindex_Range(2),Zindex_Range(1):Zindex_Range(2)-WindowOverlap)),2),3,Axial_Sampling_Spacing*PreAVE))';

    % complex first, 當是complex時, spectrogram會自動輸出完整頻譜
    
    if If_Complex == 0
        if If_YAVE == 0
            Image_Stack_Complex_Virtual_Bscan=TSBinning(abs(reshape(Image_Stack_Complex(Xindex_Range(1):Xindex_Range(2),Yindex_Range(1):Yindex_Range(2),Zindex_Range(1):Zindex_Range(2)),(Xindex_Range(2)-Xindex_Range(1)+1)*(Yindex_Range(2)-Yindex_Range(1)+1),[])),2,PreAVE);
        elseif If_YAVE == 1
            Image_Stack_Complex_Virtual_Bscan=TSBinning(squeeze(mean(abs(Image_Stack_Complex(Xindex_Range(1):Xindex_Range(2),Yindex_Range(1):Yindex_Range(2),Zindex_Range(1):Zindex_Range(2))),2)),2,PreAVE);
            Yindex_Range=[1 1];
        end
    ...
    elseif If_Complex == 1
        Image_Stack_Complex_Virtual_Bscan=TSBinning(reshape(Image_Stack_Complex(Xindex_Range(1):Xindex_Range(2),Yindex_Range(1):Yindex_Range(2),Zindex_Range(1):Zindex_Range(2)),(Xindex_Range(2)-Xindex_Range(1)+1)*(Yindex_Range(2)-Yindex_Range(1)+1),[]),2,PreAVE);
...
%%
    ...
    histogram(Color_Map);
    imwrite(Color_Map,[Data_Save_Folder sprintf('%s_FFTZmap_%gpixel_PreAVE%g.tiff',folder_list(QQQ).name,Zindex_Range(2)-Zindex_Range(1)+1,PreAVE)],'tiff');
