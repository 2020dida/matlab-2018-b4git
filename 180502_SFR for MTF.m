...
%% Data Parameter
folder_path='C:\TuanShu\180502_EM2 ref\SFR\';
...
for ImageNumber=1:TotalImageNumber
    Data_Image=double(imread([folder_path '\' folder_list(ImageNumber).name],'tiff'));%.*FF;
    %% Plot Image
    ...
    while go
        ...
        if ~isnan(Angle)
            disp(sprintf('The Edge angle is %g',Angle));
        else
...
%%
Normalized_Image=(ROI_Image-min(ROI_Image(:)))/(max(ROI_Image(:))-min(ROI_Image(:)));
BW = im2bw((ROI_Image-min(ROI_Image(:)))/(max(ROI_Image(:))-min(ROI_Image(:))),0.01);
